...
(function ($) {
  ...
  var rtsJs = {
    ...
    swiperActivation: function () {
      ...
      // Gallery slider moved to inline script in index-three.php (for fraction/progress access)
  ...
  document.addEventListener("DOMContentLoaded", function () {
    // ── Lazy-load data-bg-src via IntersectionObserver ──
    var lazyBgs = document.querySelectorAll('[data-bg-src]');
      ...
      newsletterForm.addEventListener("submit", function (e) {
        ...
        setTimeout(function () {
          ...
          if (feedbackEl) {
            ...
            feedbackEl.textContent = "Thanks for subscribing! Check your inbox soon.";
            feedbackEl.style.display = "block";
          }
...
(function(){
    ...
    function onScroll() {
        var now = Date.now();
        if (now - lastScroll < 50) return; // throttle
        lastScroll = now;
        showScrollbar();
    }
    
    // Edge trigger — mouse near right edge shows scrollbar
    function onMouseMove(e) {
        var viewW = window.innerWidth;
        var edge = 40;
        if (e.clientX > viewW - edge) {
            showScrollbar();
        }
    }
