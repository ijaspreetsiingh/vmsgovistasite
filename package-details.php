...
<div class="rts-tour-details-area pt--60 rts-section-gapBottom bg-white">
    <div class="container">
        ...
        <?php
        // Build full gallery image list: main image first (hero tile), then all gallery images.
        $galleryImages = [];
        $fallbackUrl   = packageImageUrl(null);

        // Add the package's main image first so the big hero tile shows the REAL photo
        if (!empty($mainImage) && $mainImage !== $fallbackUrl) {
            $galleryImages[] = ['url' => $mainImage, 'title' => $pkg['title'] ?? ''];
        }

        // Add gallery images
        foreach ($galleryPaths as $gi) {
            $imgUrl = packageImageUrl($gi['image_path']);
            if ($imgUrl !== $fallbackUrl) {
                $galleryImages[] = ['url' => $imgUrl, 'title' => $pkg['title'] ?? ''];
            }
        }

        // Only use the generic fallback if the package has NO images at all
        if (empty($galleryImages)) {
            $galleryImages[] = ['url' => $fallbackUrl, 'title' => $pkg['title'] ?? 'Default Package Image'];
        }
